...
{
    public class ErrorReportingTest : IClassFixture<CustomWebApplicationFactory>
    {
        private readonly ITestOutputHelper _output;
        private readonly CustomWebApplicationFactory _factory;

        public ErrorReportingTest(ITestOutputHelper output, CustomWebApplicationFactory factory)
        {
            _output = output;
            _factory = factory;
        }
        ...
        public async Task UserExceptionResponse(string index, string expectedResponse, string expectedLogPatterns)
        {
            ...
            Assert.Equal<object>(expectedResponse, $"{(int)response.StatusCode} {response.Content}");

            string apiExceptionLog = response.LogEntries.Select(e => e.ToString()).Where(e => e.Contains("ApiExceptionFilter")).Single();
        ...
        public async Task LocalizedUserException()
        {
            ...
            Assert.Equal(@"400 {""Error"":{""Message"":""TestErrorMessage 1000""}}", $"{(int)response.StatusCode} {response.Content}");
        ...
        public async Task LocalizedUserExceptionInvalidFormat()
        {
            ...
            Assert.DoesNotContain("SystemMessage", response.Content);
            Assert.DoesNotContain("Metadata", response.Content);

            Assert.DoesNotContain(@"TestErrorMessage", $"{(int)response.StatusCode} {response.Content}");
            Assert.DoesNotContain(@"1000", $"{(int)response.StatusCode} {response.Content}");
        ...
        {
            var response = await TestApiHelper.HttpPostWrite(json, _factory, _output);
            ...
            PostAsyncTest(string testName, Action<IWebHostBuilder> configureHost = null)
        {
            ...
            return await TestApiHelper.HttpPostWrite(json, _factory, _output, configureHost);
        }
    }
}
